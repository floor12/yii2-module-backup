...
Это модуль разработан чтобы организовать работу с бекапами вашего приложения силами самого Yii2
 ...
 Создание резервных копий и их восстановление для данных возможно для [MySQL](https://www.mysql.com/) и [PostgreSQL](https
 ://www.postgresql.org
 /). Для их работы необходимо иметь установленные в системе соответствующие консольные клиенты. Опыт показал, что создавать и восстанавливать базы данных 
 лучше всего используя их нативные клиенты.
...
И главный и необходимый параметр конфигурации это`configs` - в этом массиве перечислены объекты для бекапа (базы данных и папки).
 ...
 - `limit` - кол-во сохраненных копий (`0` - никогда не удалять старые копии)
  - `io` - приоритет ввода-вывода для жесткого диска во время создания резервной копии, используйте константы из `floor12\backup\models
  \IOPriority`. Значение по-умолчанию `IOPriority::IDLE`;
